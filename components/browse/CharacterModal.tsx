      ...
      >
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          ...
          {character.corePersonality && (
            <p className="text-enc-muted text-sm leading-relaxed">{character.corePersonality}</p>
          )}

          {(character.presence || character.emotionalStartingState) && (
            <div className="flex flex-wrap gap-2">
              {character.presence && (
                <span className="text-enc-dim text-xs border border-enc-border px-2 py-0.5 rounded">
                  {character.presence}
                </span>
              )}
              {character.emotionalStartingState && (
                <span className="text-enc-dim text-xs border border-enc-border px-2 py-0.5 rounded">
                  {character.emotionalStartingState}
                </span>
              )}
